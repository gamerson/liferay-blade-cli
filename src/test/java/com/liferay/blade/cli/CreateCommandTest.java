...
public class CreateCommandTest {
	...
	public void testCreateApi() throws Exception {
		...
		try (Jar jar = new Jar(new File(projectPath + "/build/libs/foo-1.0.0.jar"))) {
			...
			Assert.assertEquals("foo.api;version=\"1.0.0\"", attributes.getValue("Export-Package"));
		}
	...
	public void testCreateGradleServiceBuilderDefault() throws Exception {
		...
		_contains(
			_checkFileExists(projectPath + "/guestbook-service/build.gradle"),
		...
		try (JarFile serviceJarFile = new JarFile(serviceJar)) {
			Attributes attributes = serviceJarFile.getManifest().getMainAttributes();

			String springContext = attributes.getValue("Liferay-Spring-Context");
	...
	private void _contains(String content, String pattern) throws Exception {
		...
		Assert.assertTrue(p.matcher(content).matches());
	}
	...
	private File _testdir = IO.getFile("build/test");
